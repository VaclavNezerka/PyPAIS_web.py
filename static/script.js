...
function validateThresholds(id) {
    ...
    const comp_id = id.includes("Value")
        ? id.replace("Value", "Slider")
    ...
    if (id.toLowerCase().includes("min")) {
        ...
        if (val > document.getElementById(mm_id).value) {
            ...
            document.getElementById(mm_id.replace("Value", "Slider")).value =
                val
            document
                .getElementById(mm_id.replace("Value", "Slider"))
...
function processImage() {
    return new Promise((resolve, reject) => {
        ...
        formData.append(
            "minThreshold0",
            document.getElementById("minThresholdSlider0").value,
        )
        formData.append(
            "maxThreshold0",
            document.getElementById("maxThresholdSlider0").value,
        )
        formData.append(
            "minThreshold1",
            document.getElementById("minThresholdSlider1").value,
        )
        formData.append(
            "maxThreshold1",
            document.getElementById("maxThresholdSlider1").value,
        )
        formData.append(
            "entropyMinThreshold",
            document.getElementById("entropyMinThresholdSlider").value,
        )
        formData.append(
            "entropyMaxThreshold",
            document.getElementById("entropyMaxThresholdSlider").value,
        )
...
function drawHistogram(canvas, histogram, minThreshold, maxThreshold) {
    ...
    for (let k = 0; k < minThreshold.length; k++) {
        ...
        for (let i = start; i < end; i++) {
            ...
            ctx.fillStyle =
                i >= minThreshold[k] && i <= maxThreshold[k] ? "red" : "black"
            ctx.fill()
        }
    }
}
...
function createIntensityHistogram() {
    return new Promise((resolve, reject) => {
        ...
        img.onload = function () {
            ...
            grayscaleImageData = ctx.getImageData(
                0,
                0,
                canvas.width,
                canvas.height,
...
function loadExperiment(id) {
    ...
    fetch("/load-experiment/" + String(id), { method: "GET" })
        ...
        .then((data) => {
            ...
            document.getElementById("minThresholdSlider0").value =
                data.minThreshold0
            document.getElementById("minThresholdValue0").value =
                data.minThreshold0
            document.getElementById("maxThresholdSlider0").value =
                data.maxThreshold0
            document.getElementById("maxThresholdValue0").value =
                data.maxThreshold0
            document.getElementById("minThresholdSlider1").value =
                data.minThreshold1
            document.getElementById("minThresholdValue1").value =
                data.minThreshold1
            document.getElementById("maxThresholdSlider1").value =
                data.maxThreshold1
            document.getElementById("maxThresholdValue1").value =
                data.maxThreshold1
            document.getElementById("entropyMinThresholdSlider").value =
                data.entropyMinThreshold
            document.getElementById("entropyMinThresholdValue").value =
                data.entropyMinThreshold
            document.getElementById("entropyMaxThresholdSlider").value =
                data.entropyMaxThreshold
            document.getElementById("entropyMaxThresholdValue").value =
                data.entropyMaxThreshold
            ...
            uploadedImageURL_nobg = URL.createObjectURL(
                base64toBlob(data.nobg, "image/png"),
            )
            uploadedImageURL_color_blur = URL.createObjectURL(
                base64toBlob(data.color_blur, "image/png"),
            )
            uploadedImageURL_gray_blur = URL.createObjectURL(
                base64toBlob(data.gray_blur, "image/png"),
            )
            uploadedImageURL_nobg_blur = URL.createObjectURL(
                base64toBlob(data.nobg_blur, "image/png"),
            )
        })
        .then(() => {
            createIntensityHistogram()
...
document
    .getElementById("sharpnessCheckbox")
    .addEventListener("change", changeSharpness)
document
    .getElementById("redOverlayCheckbox")
    .addEventListener("change", changeRedOverlay)
        ...
        if (document.getElementById("expertGuess").value === "") {
            ...
            fetch("/evaluate-asphalt" + uniqueQuery, { method: "POST" })
                ...
                .then((response) => {
                    ...
                    alert(
                        "Evaluation completed. Check the console for the results." +
                            "\n" +
                            "Evaluation results: " +
                            displayNum +
                            "%",
...
document.addEventListener("keydown", function (event) {
    ...
    switch (eventKey) {
        case "a":
            ...
            break
        case "b":
            document.getElementById("sharpnessCheckbox").checked =
                !document.getElementById("sharpnessCheckbox").checked
            changeSharpness()
            break
        case "c":
...
viewer.addHandler("viewport-change", function () {
    ...
    const imageCenter = viewer.viewport.viewportToImageCoordinates(
        center.x,
        center.y,
    )
...
function drawPreview(imagePoint) {
    ...
    const pointSuggestions = pointHistory.concat({
        x: imagePoint.x,
        y: imagePoint.y,
    })
