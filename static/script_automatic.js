...
uploadedImageOverlay_bg.onload = function() {
    displayed_mask('overlayCanvasBackground', this, labelSettings.background.fill);
};
...
function get_overlay_masks() {
    ...
    .then(data => {
        asphaltMaskURL = URL.createObjectURL(base64toBlob(data.asphalt_mask, 'image/png'));
        aggregateMaskURL = URL.createObjectURL(base64toBlob(data.aggregate_mask, 'image/png'));
        backgroundMaskURL = URL.createObjectURL(base64toBlob(data.background_mask, 'image/png'));
    })
    .catch( error => {
...
function get_image_color() {
    ...
    .then(response => response.json())
    .then(data => {
       uploadedImageURL_gray = URL.createObjectURL(base64toBlob(data.color,'image/png'))
    })
    .catch( error => {
...
function overlayMask(baseUrl, maskUrl) {
  ...
  Promise.all([
    new Promise(r => { base.onload = r; base.src = baseUrl; }),
    new Promise(r => { mask.onload = r; mask.src = maskUrl; }),
  ]).then(() => {
...
function getImageType() {
    let imageType = document.getElementById('imageType').value;
        ...
        if (displayAsphaltMask) {
            ...
            document.getElementById('overlayCanvasAsphalt').style.opacity = labelSettings.mask_opacity;
        } else {
            document.getElementById('overlayCanvasAsphalt').style.display = 'none';
        }
        if (displayAggregateMask) {
            uploadedImageOverlay_aggregate.src = uploadedImageURL_mask_aggregate;
            document.getElementById('overlayCanvasAggregate').style.display = 'block';
            document.getElementById('overlayCanvasAggregate').style.opacity = labelSettings.mask_opacity;
        } else {
            document.getElementById('overlayCanvasAggregate').style.display = 'none';
        }
        if (displayBackgroundMask) {
            uploadedImageOverlay_bg.src = uploadedImageURL_mask_bg;
            document.getElementById('overlayCanvasBackground').style.display = 'block';
            document.getElementById('overlayCanvasBackground').style.opacity = labelSettings.mask_opacity;
        } else {
            document.getElementById('overlayCanvasBackground').style.display = 'none';
        }
...
document.getElementById('opacitySlider').addEventListener('change', changeOverlayOpacity);
